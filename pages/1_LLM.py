...
eval_questions = st.session_state.get('eval_questions', [])
eval_answers = st.session_state.get('eval_answers', [])
doc_path = st.session_state.get('doc_path', 'docs/')
...
# Print the list of files information
...
if st.button('Generate Embeddings'):
    ...
    vectorstore = load_local_vectordb_using_qdrant("testing_arabic", embeddings)
...
st.write(
        """
        Prompt: أنت مساعد لمهام الإجابة عن الأسئلة و الاستشارات المالية. استخدم السياقات المسترجعة التالية للإجابة على السؤال.
إذا كنت لا تعرف الإجابة، فقط قل أنك لا تعرف. استخدم ثلاث جمل كحد أقصى واحرص على أن تكون الإجابة مختصرة.
        Question: {question}
        Context: {context}
        Answer:
        """
